...
function FilterInput({ value, onChange, onSubmit, inputLabel, i18n }) {
  ...
  return u("section", { className: "uppy-ProviderBrowser-searchFilter", children: [u("input", { className: "uppy-u-reset uppy-ProviderBrowser-searchFilterInput", type: "search", "aria-label": inputLabel, placeholder: inputLabel, value, onInput: (e2) => onChange(e2.target.value), form: formId, "data-uppy-super-focusable": true }), u("svg", { "aria-hidden": "true", focusable: "false", className: "uppy-c-icon uppy-ProviderBrowser-searchFilterIcon", width: "12", height: "12", viewBox: "0 0 12 12", children: u("path", { d: "M8.638 7.99l3.172 3.172a.492.492 0 1 1-.697.697L7.91 8.656a4.977 4.977 0 0 1-2.983.983C2.206 9.639 0 7.481 0 4.819 0 2.158 2.206 0 4.927 0c2.721 0 4.927 2.158 4.927 4.82a4.74 4.74 0 0 1-1.216 3.17zm-3.71.685c2.176 0 3.94-1.726 3.94-3.856 0-2.129-1.764-3.855-3.94-3.855C2.75.964.984 2.69.984 4.819c0 2.13 1.765 3.856 3.942 3.856z" }) }), value && u("button", { className: "uppy-u-reset uppy-ProviderBrowser-searchFilterReset", type: "button", "aria-label": i18n("resetFilter"), onClick: () => onChange(""), children: u("svg", { "aria-hidden": "true", focusable: "false", className: "uppy-c-icon", viewBox: "0 0 19 19", children: u("path", { d: "M17.318 17.232L9.94 9.854 9.586 9.5l-.354.354-7.378 7.378h.707l-.62-.62v.706L9.318 9.94l.354-.354-.354-.354L1.94 1.854v.707l.62-.62h-.706l7.378 7.378.354.354.354-.354 7.378-7.378h-.707l.622.62v-.706L9.854 9.232l-.354.354.354.354 7.378 7.378.708-.707-7.38-7.378v.708l7.38-7.38.353-.353-.353-.353-.622-.622-.353-.353-.354.352-7.378 7.38h.708L2.56 1.23 2.208.88l-.353.353-.622.62-.353.355.352.353 7.38 7.38v-.708l-7.38 7.38-.353.353.352.353.622.622.353.353.354-.353 7.38-7.38h-.708l7.38 7.38z" }) }) })] });
}
...
function ListItem({ file, openFolder, className, isDisabled, restrictionError, toggleCheckbox, showTitles, i18n }) {
  return u("li", { className, title: file.status !== "checked" && restrictionError ? restrictionError : void 0, children: [u("input", {
    ...
    u("button", { type: "button", className: "uppy-u-reset uppy-c-btn uppy-ProviderBrowserItem-inner", onClick: () => openFolder(file.id), "aria-label": i18n("openFolderNamed", {
      name: file.data.name ?? i18n("unnamed")
    }), children: [u("div", { className: "uppy-ProviderBrowserItem-iconWrap", children: u(ItemIcon, { itemIconString: file.data.icon }) }), showTitles && file.data.name ? u("span", { className: "uppy-truncate-text", children: file.data.name }) : i18n("unnamed")] })
  ) : (
    // label for a checkbox
    u("label", { htmlFor: file.id, className: "uppy-u-reset uppy-ProviderBrowserItem-inner", children: [u("div", { className: "uppy-ProviderBrowserItem-iconWrap", children: u(ItemIcon, { itemIconString: file.data.icon }) }), showTitles && u("span", { className: "uppy-truncate-text", children: file.data.name ?? i18n("unnamed") })] })
  )] });
}
...
function Browser(props) {
  ...
  if (virtualList) {
    return u("div", { className: "uppy-ProviderBrowser-body", children: u(VirtualList, { className: "uppy-ProviderBrowser-list", data: displayedPartialTree, renderRow: renderItem, rowHeight: 35.5 }) });
  }
...
var getNumberOfSelectedFiles = (partialTree) => {
  const checkedLeaves = partialTree.filter((item) => {
    ...
    if (item.type === "folder" && item.status === "checked") {
      const doesItHaveChildren = partialTree.some((i2) => i2.type !== "root" && i2.parentId === item.id);
...
var addFiles = (companionFiles, plugin, provider) => {
  const uppyFiles = companionFiles.map((f2) => companionFileToUppyFile_default(f2, plugin, provider));
  const filesToAdd = [];
...
var getBreadcrumbs = (partialTree, currentFolderId) => {
  let folder = partialTree.find((f2) => f2.id === currentFolderId);
  let breadcrumbs = [];
  while (true) {
    ...
    folder = partialTree.find((f2) => f2.id === currentParentId);
  }
  return breadcrumbs;
};
...
var getPath = (partialTree, id, cache) => {
  ...
  if (cache[sId])
    return cache[sId];
  const file = partialTree.find((f2) => f2.id === id);
  if (file.type === "root")
    return [];
...
var getCheckedFilesWithPaths = (partialTree) => {
  ...
  const companionFilesWithInjectedPaths = checkedFiles.map((file) => {
    ...
    const firstCheckedFolderIndex = absFolders.findIndex((i2) => i2.type === "folder" && i2.status === "checked");
    const relFolders = absFolders.slice(firstCheckedFolderIndex);
    const absDirPath = `/${absFolders.map((i2) => i2.data.name).join("/")}`;
    const relDirPath = relFolders.length === 1 ? (
      ...
      void 0
    ) : relFolders.map((i2) => i2.data.name).join("/");
    return {
      ...file.data,
      absDirPath,
      relDirPath
...
var PriorityQueue = class {
  ...
  enqueue(run, options) {
    ...
    const index = lowerBound(this.#queue, element, (a2, b3) => b3.priority - a2.priority);
...
var recursivelyFetch = async (queue, poorTree, poorFolder, apiList, validateSingleFile) => {
  ...
  const newFolders = items.filter((i2) => i2.isFolder === true);
  const newFiles = items.filter((i2) => i2.isFolder === false);
...
var afterFill = async (partialTree, apiList, validateSingleFile, reportProgress) => {
  ...
  queue.on("completed", () => {
    const nOfFilesChecked = poorTree.filter((i2) => i2.type === "file" && i2.status === "checked").length;
...
var afterOpenFolder = (oldPartialTree, discoveredItems, clickedFolder, currentPagePath, validateSingleFile) => {
  const discoveredUniqueItems = discoveredItems.filter((i2) => !oldPartialTree.find((f2) => f2.id === i2.requestPath));
  const discoveredFolders = discoveredUniqueItems.filter((i2) => i2.isFolder === true);
  const discoveredFiles = discoveredUniqueItems.filter((i2) => i2.isFolder === false);
...
var afterScrollFolder = (oldPartialTree, currentFolderId, items, nextPagePath, validateSingleFile) => {
  const currentFolder = oldPartialTree.find((i2) => i2.id === currentFolderId);
  const newFolders = items.filter((i2) => i2.isFolder === true);
  const newFiles = items.filter((i2) => i2.isFolder === false);
...
var SearchResultItem = ({ i18n, item, toggleCheckbox, openFolder }) => {
  ...
  return u("li", { className: (0, import_classnames3.default)("uppy-ProviderBrowserItem", { "uppy-ProviderBrowserItem--disabled": isDisabled }, { "uppy-ProviderBrowserItem--noPreview": item.data.icon === "video" }, { "uppy-ProviderBrowserItem--is-checked": item.status === "checked" }, { "uppy-ProviderBrowserItem--is-partial": item.status === "partial" }), title: ("restrictionError" in item ? item.restrictionError : void 0) ?? void 0, children: [u("input", { type: "checkbox", className: "uppy-u-reset uppy-ProviderBrowserItem-checkbox", onChange: () => toggleCheckbox(item, false), checked: item.status === "checked", "aria-label": item.data.name ?? i18n("unnamed"), disabled: isDisabled, "data-uppy-super-focusable": true }), u("button", { type: "button", className: "uppy-u-reset uppy-c-btn uppy-ProviderBrowserItem-inner", disabled: isDisabled, "aria-label": item.data.name, onClick: () => {
    if (item.data.isFolder) {
      openFolder(item.id);
    }
  }, children: [u("div", { className: "uppy-ProviderBrowserItem-iconWrap", children: u(ItemIcon, { itemIconString: item.data.icon }) }), item.data.name ?? i18n("unnamed")] })] });
};
...
var GlobalSearchView = ({ searchResults, toggleCheckbox, openFolder, i18n }) => {
  if (searchResults.length === 0) {
    return u("div", { className: "uppy-Provider-empty", children: i18n("noFilesFound") });
  }
  return u("div", { className: "uppy-ProviderBrowser-body", children: u("ul", { className: "uppy-ProviderBrowser-list", children: searchResults.map((item) => u(SearchResultItem_default, { i18n, item, toggleCheckbox, openFolder }, item.id)) }) });
};
...
function Header(props) {
  return u("div", { className: "uppy-ProviderBrowser-header", children: u("div", { className: (0, import_classnames4.default)("uppy-ProviderBrowser-headerBar", !props.showBreadcrumbs && "uppy-ProviderBrowser-headerBar--simple"), children: [props.showBreadcrumbs && u(Breadcrumbs, { openFolder: props.openFolder, breadcrumbs: props.breadcrumbs, breadcrumbsIcon: props.pluginIcon?.(), title: props.title, i18n: props.i18n }), u(User, { logout: props.logout, username: props.username, i18n: props.i18n })] }) });
}
...
var ProviderView = class _ProviderView {
  ...
  constructor(plugin, opts) {
    ...
    this.opts = { ...defaultOptions4, ...opts };
    ...
    this.#searchDebounced = (0, import_debounce.default)(this.#search, wait, debounceOpts);
  }
  resetPluginState() {
  ...
  async #search() {
    ...
    const currentFolder = partialTree.find((i2) => i2.id === currentFolderId);
    ...
    await this.#withAbort(async (signal) => {
      ...
      for (const file of items) {
        ...
        const segments = decodedPath.split("/").filter((s2) => s2.length > 0);
        ...
        segments.forEach((segment, index, arr) => {
          ...
          const existingNode = newPartialTree.find((n2) => n2.id === encodedPath && n2.type !== "root");
          if (existingNode) {
  ...
  #renderSearchResults() {
    ...
    return u(GlobalSearchView_default, { searchResults, openFolder: this.openSearchResultFolder, toggleCheckbox: this.toggleCheckbox, i18n });
  }
  render(state, viewOptions = {}) {
    ...
    return u("div", { className: (0, import_classnames5.default)("uppy-ProviderBrowser", `uppy-ProviderBrowser-viewType--${opts.viewType}`), children: [u(Header, { showBreadcrumbs: opts.showBreadcrumbs, openFolder: this.openFolder, breadcrumbs, pluginIcon, title: this.plugin.title, logout: this.logout, username, i18n }), opts.showFilter && u(FilterInput_default, { value: searchString, onChange: (s2) => this.onSearchInput(s2), onSubmit: () => {
    }, inputLabel: i18n("filter"), i18n }), searchResults ? this.#renderSearchResults() : u(Browser_default, { toggleCheckbox: this.toggleCheckbox, displayedPartialTree: this.getDisplayedPartialTree(), openFolder: this.openFolder, virtualList: opts.virtualList, noResultsLabel: i18n("noFilesFound"), handleScroll: this.handleScroll, viewType: opts.viewType, showTitles: opts.showTitles, i18n: this.plugin.uppy.i18n, isLoading: loading, utmSource: "Companion" }), u(FooterActions, { partialTree, donePicking: this.donePicking, cancelSelection: this.cancelSelection, i18n, validateAggregateRestrictions: this.validateAggregateRestrictions })] });
  }
};
...
// node_modules/@uppy/provider-views/lib/SearchView.js
function SearchView({ value, onChange, onSubmit, inputLabel, loading = false, children }) {
  const { formId } = useSearchForm(onSubmit);
  return u("section", { className: "uppy-SearchProvider", children: [u("input", { className: "uppy-u-reset uppy-c-textInput uppy-SearchProvider-input", type: "search", "aria-label": inputLabel, placeholder: inputLabel, value, onInput: (e2) => onChange(e2.target.value), form: formId, disabled: loading, "data-uppy-super-focusable": true }), u("button", { disabled: loading, className: "uppy-u-reset uppy-c-btn uppy-c-btn-primary uppy-SearchProvider-searchButton", type: "submit", form: formId, children })] });
}
var SearchView_default = SearchView;
...
var SearchProviderView = class {
  ...
  async handleScroll(event) {
    ...
    const root = partialTree.find((i2) => i2.type === "root");
    if (shouldHandleScroll_default(event) && !this.isHandlingScroll && root.nextPagePath) {
      ...
      try {
        ...
        const oldItems = partialTree.filter((i2) => i2.type !== "root");
        const newPartialTree = [
          newRoot,
          ...oldItems,
          ...response.items.map((item) => ({
  ...
  render(state, viewOptions = {}) {
    ...
    const opts = { ...this.opts, ...viewOptions };
    if (isInputMode) {
      return u(SearchView_default, { value: searchString, onChange: this.setSearchString, onSubmit: this.search, inputLabel: i18n("enterTextToSearch"), children: i18n("searchImages") });
    }
    return u("div", { className: (0, import_classnames6.default)("uppy-ProviderBrowser", `uppy-ProviderBrowser-viewType--${opts.viewType}`), children: [opts.showFilter && u(FilterInput_default, { value: searchString, onChange: this.setSearchString, onSubmit: this.search, inputLabel: i18n("search"), i18n }), u(Browser_default, { toggleCheckbox: this.toggleCheckbox, displayedPartialTree: this.getDisplayedPartialTree(), handleScroll: this.handleScroll, openFolder: async () => {
...
var c = class _c {
  ...
  getString(e2 = 0, t2 = this.byteLength) {
    ...
    return i2 = s2, d2 ? d2.decode(i2) : a ? Buffer.from(i2).toString("utf8") : decodeURIComponent(escape(u2(i2)));
    var i2;
  }
...
async function O(e2, t2, s2, i2) {
  return b2.has(s2) ? v(e2, t2, s2) : i2 ? (async function(e3, t3) {
    let s3 = await t3(e3);
    return new c(s3);
  })(e2, i2) : void l(`Parser ${s2} is not loaded`);
}
async function v(e2, t2, s2) {
  let i2 = new (b2.get(s2))(e2, t2);
...
var H2 = class {
  ...
  async read(e2) {
    this.file = await (function(e3, t2) {
      return "string" == typeof e3 ? k2(e3, t2) : s && !i && e3 instanceof HTMLImageElement ? k2(e3.src, t2) : e3 instanceof Uint8Array || e3 instanceof ArrayBuffer || e3 instanceof DataView ? new c(e3) : s && e3 instanceof Blob ? O(e3, t2, "blob", A2) : void l("Invalid input argument");
    })(e2, this.options);
  }
  setup() {
...
var ee = class extends class {
  ...
  findAppSegmentsInRange(e2, t2) {
    t2 -= 2;
    let s2, i2, n2, r2, a2, h2, { file: f2, findAll: l2, wanted: o2, remaining: u3, options: d3 } = this;
    for (; e2 < t2; e2++) if (255 === f2.getUint8(e2)) {
      if (s2 = f2.getUint8(e2 + 1), Q(s2)) {
        if (i2 = f2.getUint16(e2 + 2), n2 = Z(f2, e2, i2), n2 && o2.has(n2) && (r2 = y2.get(n2), a2 = r2.findPosition(f2, e2), h2 = d3[n2], a2.type = n2, this.appSegments.push(a2), !l2 && (r2.multiSegment && h2.multiSegment ? (this.unfinishedMultiSegment = a2.chunkNumber < a2.chunkCount, this.unfinishedMultiSegment || u3.delete(n2)) : u3.delete(n2), 0 === u3.size))) break;
        ...
        d3.recordJpegSegments && this.jpegSegments.push({ offset: e2, length: i2, marker: s2 }), e2 += i2 + 1;
      }
    }
    return e2;
  }
...
b2.set("blob", class extends Oe {
  ...
  async _readChunk(e2, t2) {
    let s2 = t2 ? e2 + t2 : void 0, i2 = this.input.slice(e2, s2), n2 = await A2(i2);
...
var AddFilesPanel = (props) => {
  return u("div", { className: (0, import_classnames7.default)("uppy-Dashboard-AddFilesPanel", props.className), "data-uppy-panelType": "AddFiles", "aria-hidden": !props.showAddFilesPanel, children: [u("div", { className: "uppy-DashboardContent-bar", children: [u("div", {
...
function EditorPanel(props) {
  ...
  return u("div", { className: (0, import_classnames8.default)("uppy-DashboardContent-panel", props.className), role: "tabpanel", "data-uppy-panelType": "FileEditor", id: "uppy-DashboardContent-panel--editor", children: [u("div", { className: "uppy-DashboardContent-bar", children: [u("div", {
    className: "uppy-DashboardContent-title",
    // biome-ignore lint/a11y/useSemanticElements: ...
    role: "heading",
    "aria-level": 1,
...
function FileCard(props) {
  ...
  return (
    ...
    u("div", { className: (0, import_classnames9.default)("uppy-Dashboard-FileCard", className), "data-uppy-panelType": "FileCard", onDragOver: ignoreEvent_default, onDragLeave: ignoreEvent_default, onDrop: ignoreEvent_default, onPaste: ignoreEvent_default, ref: domRef, children: [u("div", { className: "uppy-DashboardContent-bar", children: [u("div", {
      className: "uppy-DashboardContent-title",
      // biome-ignore lint/a11y/useSemanticElements: ...
      role: "heading",
      "aria-level": 1,
...
var renderFileSize = (props) => props.file.size && u("div", { className: "uppy-Dashboard-Item-statusSize", children: (0, import_prettier_bytes.default)(props.file.size) });
...
var FileItem = class extends x {
  ...
  render() {
    ...
    const dashboardItemClass = (0, import_classnames10.default)({
...
function FileList({ id, i18n, uppy, files, resumableUploads, hideRetryButton, hidePauseResumeButton, hideCancelButton, showLinkToFileUploadResult, showRemoveButtonAfterComplete, metaFields, isSingleFile, toggleFileCard, handleRequestThumbnail, handleCancelThumbnail, recoveredState, individualCancellation, itemsPerRow, openFileEditor, canEditFile, toggleAddFilesPanel, containerWidth, containerHeight }) {
  ...
  return u(VirtualList, { class: "uppy-Dashboard-files", role: "list", data: rows, renderRow, rowHeight });
}
...
var Informer = class extends x {
  render() {
    ...
    return u("div", { className: "uppy uppy-Informer", children: u(TransitionGroup_default, { children: info.map((info2) => u(FadeIn, { children: u("p", { role: "alert", children: [info2.message, " ", info2.details && // biome-ignore lint/a11y/useKeyWithClickEvents: don't think it's needed
    u("span", { "aria-label": info2.details, "data-microtip-position": "top-left", "data-microtip-size": "medium", role: "tooltip", onClick: () => alert(`${info2.message} 
...
function PickerPanelContent({ activePickerPanel, className, hideAllPanels, i18n, state, uppy }) {
  const ref = A(null);
  return u("div", { className: (0, import_classnames11.default)("uppy-DashboardContent-panel", className), role: "tabpanel", "data-uppy-panelType": "PickerPanel", id: `uppy-DashboardContent-panel--${activePickerPanel.id}`, onDragOver: ignoreEvent_default, onDragLeave: ignoreEvent_default, onDrop: ignoreEvent_default, onPaste: ignoreEvent_default, children: [u("div", { className: "uppy-DashboardContent-bar", children: [u("div", {
    className: "uppy-DashboardContent-title",
    // biome-ignore lint/a11y/useSemanticElements: ...
    role: "heading",
    "aria-level": 1,
...
function Slide({ children }) {
  ...
  return K(cachedChildren, {
    className: (0, import_classnames12.default)(className, cachedChildren.props.className)
  });
}
...
function UploadBtn(props) {
  ...
  const uploadBtnClassNames = (0, import_classnames13.default)("uppy-u-reset", "uppy-c-btn", "uppy-StatusBar-actionBtn", "uppy-StatusBar-actionBtn--upload", {
...
function UploadNewlyAddedFiles(props) {
  ...
  const uploadBtnClassNames = (0, import_classnames13.default)("uppy-u-reset", "uppy-c-btn", "uppy-StatusBar-actionBtn", "uppy-StatusBar-actionBtn--uploadNewlyAdded");
...
function StatusBarUI({ newFiles, allowNewUpload, isUploadInProgress, isAllPaused, resumableUploads, error, hideUploadButton = void 0, hidePauseResumeButton = false, hideCancelButton = false, hideRetryButton = false, recoveredState, uploadState, totalProgress, files, supportsUploadProgress, hideAfterFinish = false, isSomeGhost, doneButtonHandler = void 0, isUploadStarted, i18n, startUpload, uppy, isAllComplete, hideProgressDetails = void 0, numUploads, complete, totalSize, totalETA, totalUploadedSize }) {
  ...
  const progressClassNames = (0, import_classnames14.default)("uppy-StatusBar-progress", {
    "is-indeterminate": getIsIndeterminate()
  });
  const statusBarClassNames = (0, import_classnames14.default)("uppy-StatusBar", `is-${uploadState}`, { "has-ghosts": isSomeGhost });
  const progressBarStateEl = (() => {
    switch (uploadState) {
      ...
      case STATE_UPLOADING:
        return u(ProgressBarUploading, { i18n, supportsUploadProgress, totalProgress, hideProgressDetails, isUploadStarted, isAllComplete, isAllPaused, newFiles, numUploads, complete, totalUploadedSize, totalSize, totalETA, startUpload });
      default:
        return null;
    }
  })();
...
var StatusBar = class extends x {
  ...
  componentDidMount() {
    ...
    this.#previousUploadedBytes = this.props.uppy.getFiles().reduce((pv, file) => pv + (file.progress.bytesUploaded || 0), 0);
  ...
  render() {
    ...
    return u(StatusBarUI, { error, uploadState: getUploadingState2(error, isAllComplete, recoveredState, files || {}), allowNewUpload, totalProgress, totalSize, totalUploadedSize, isAllComplete, isAllPaused, isUploadStarted, isUploadInProgress, isSomeGhost, recoveredState, complete: completeFiles.length, newFiles: newFilesOrRecovered.length, numUploads: startedFiles.length, totalETA, files, i18n: this.props.i18n, uppy: this.props.uppy, startUpload: this.startUpload, doneButtonHandler: this.props.doneButtonHandler, resumableUploads, supportsUploadProgress, hideProgressDetails: this.props.hideProgressDetails, hideUploadButton: this.props.hideUploadButton, hideRetryButton: this.props.hideRetryButton, hidePauseResumeButton: this.props.hidePauseResumeButton, hideCancelButton: this.props.hideCancelButton, hideAfterFinish: this.props.hideAfterFinish });
  }
};
...
function Dashboard(props) {
  ...
  const dashboardClassName = (0, import_classnames15.default)({
    "uppy-Dashboard": true,
...
var Dashboard2 = class extends UIPlugin_default {
  ...
  #disabledNodes;
  modalName = `uppy-Dashboard-${nanoid2()}`;
